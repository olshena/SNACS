...
## Make hash calls
This is the final step for demultiplexing the hashes. The cluster information from the mutation data and the hash antibody data are used to make hash calls. Each cell is assigned the ID of one of the hashes or called a multiplet when it is a mixture of hashes. Expecting the mutation data of cells with similar hash profiles to cluster together, the cells are split into sub-clusters and each cluster is assigned a hash ID if enough proportion of cells in the cluster have signal above the background of that hash.
...
createHeatmap(snacsObj,
   cell_anno_var=c("hashCallRnd2","hashCallRnd1",snacsObj$annHash$hashNames,"clustRankedSNPs_hclust"),
   cell_anno_name=c("hashCallRnd2","hashCallRnd1",snacsObj$annHash$hashNames,"cluster"),
   outputFormat="")
